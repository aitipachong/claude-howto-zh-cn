...
#### `args` 形式：直接 exec，不走 shell

从 `v2.1.139` 开始，command hook 还支持 `args` 形式，直接用 `execve()` 拉起目标二进制，不经过 shell 解析。
...
### `continueOnBlock`：让 Claude 看到 hook 的阻断反馈后再调整

从 `v2.1.139` 开始，`PostToolUse` hook 可以配 `continueOnBlock: true`。默认情况下 hook 返回 `"decision": "block"` 会直接终止当前 turn；加上这个字段后，Claude 会把阻断原因当成一个 `tool_result` 读回来，再决定如何重试或换方案。
...
从 `v2.1.133+` 开始，hook 输入和 Bash 子进程环境里都能拿到当前 effort 信息。
这对你想做“高 effort 时多跑一层校验，低 effort 时只做轻量检查”的场景很有用。

你可以关注两类入口：

- hook 输入 JSON 里的 `effort.level`
- Bash 子进程环境变量里的 `CLAUDE_EFFORT`
...
### `SessionStart` 可以重扫 skills 并设置 session 标题

从 `v2.1.152+` 开始，`SessionStart` hook 可以通过 stdout JSON 做两件很实用的事：
...
- `reloadSkills: true`：在当前 session 里重新扫描 skills，效果类似 `/reload-skills`
