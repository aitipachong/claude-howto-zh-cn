...
从 `v2.1.157` 起，可以直接用下面的命令在 `.claude/skills` 中创建本地 plugin：

```bash
claude plugin init my-plugin
```

放在 `.claude/skills` 里的 plugin 会自动加载，不需要先发布到 marketplace。这里的 `claude plugin init <name>`、`.claude/skills` 和 plugin 名都属于可执行标识，不要改成中文。
