...
这点对自动化很重要：`acceptEdits` 只是减少普通编辑确认，不代表可以静默修改会影响命令执行链路的配置。
...
`v2.1.154+` 增加了 `/workflows`，用于查看正在运行和已经完成的 dynamic workflows。

你可以把 dynamic workflows 理解成“更可控的多代理编排”：适合把几十个甚至上百个后台 subagents 按 fan-out、pipeline、并行阶段等方式组织起来，用在大规模审查、迁移、全仓扫描这类任务上。

对中文用户来说，先记住两个入口就够了：

- `/workflows`：查看 workflow 运行记录
- `ultracode`：不是模型 effort level，而是会发送 `xhigh` 并让 Claude 编排 dynamic workflows 的会话级模式
...
### `EnterWorktree`：在 session 中切换 worktree

从 `v2.1.157` 起，`EnterWorktree` 可以在同一个 session 中切换 Claude 管理的 worktree。Claude 管理的 worktree 在 agent 结束后也会保持 unlocked，方便你用 `git worktree remove` 或 `git worktree prune` 做后续清理。
...
`ultracode` 也容易误解：它不是一个模型 effort level，而是会发送 `xhigh` 并让 Claude 编排 dynamic workflows 的会话级能力。
